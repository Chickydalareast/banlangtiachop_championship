...
function makeFallbackCompare(
  ...
  return (
    ...
    const teamA =
      teamById.get(teamAId);
    const teamB =
      teamById.get(teamBId);

    if (!teamA || !teamB) {
      return teamAId.localeCompare(
        teamBId,
        "en",
      );
    }
...
export function calculateStandings(
  ...
  if (
    ...
    throw new Error(
      "qualificationCount must be a positive integer.",
    );
  }
  ...
  for (const match of matches) {
    ...
    if (matchByPair.has(key)) {
      throw new Error(
        `Duplicate finished match for pair: ${key}`,
      );
    }
    ...
    teamAStanding.gamesFor +=
      scoreA;
    teamAStanding.gamesAgainst +=
      scoreB;
    teamBStanding.gamesFor +=
      scoreB;
    teamBStanding.gamesAgainst +=
      scoreA;
  ...
  const fallbackCompare =
    makeFallbackCompare(teamById);
  const primaryGroups =
    groupByPrimaryCriteria(
      Array.from(
        baseByTeamId.values(),
      ),
      fallbackCompare,
  ...
  for (const block of blocks) {
    for (
      const teamId of block.teamIds
    ) {
      const team =
        teamById.get(teamId);
      const base =
        baseByTeamId.get(teamId);

      if (!team || !base) {
        throw new Error(
          `Missing standings data for team: ${teamId}`,
        );
      }
